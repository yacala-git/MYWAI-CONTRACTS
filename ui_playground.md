...
## Trip-map render fixes + day-plan placement (2026-07-12 — commit 2029a87)
- **maplibre-gl.css is REQUIRED and lazily imported** in `tripmap/render/adapter.ts init()` alongside the engine (`await import('maplibre-gl/dist/maplibre-gl.css')` + `inkOverrides.css`). Without it markers render in document flow (the "flying landmarks" bug). Entry-purity invariant unchanged (no maplibre in entry chunks). Runtime guards warn if the canvas or a marker computes `position !== 'absolute'`.
- **Marker roots must NEVER carry inline `position`** — the element handed to `maplibre.Marker` relies on `.maplibregl-marker { position:absolute }`; any positioning context for badge/foot/silhouette lives on an INNER wrapper div (TripMapCanvas `markerElement`/`vantageMarkerElement`).
- **Camera fit:** `clampPadding` caps padding at ≤50% per axis (MapLibre fitBounds silently no-ops when padding > container); `shouldDeferFit` defers when the container <120px (sheet mid-animation) and the mount ResizeObserver re-invokes the fit once usable. Fit source remains SERVER bbox only, programmatic fits animate:false.
- **Day-plan placement:** hotel SketchCards no longer render the in-card day-plan on full-shape trips — `hideDayPlan` on SketchCard, forwarded by SketchFrameBlock, gated on `builtTripShape` (snapshotted in `send()` like the canvasStage flight decision; reset() re-baselines). The day-plan's home is the activities STAGE (entered on Confirm trip); pre-Confirm the itinerary is reachable via the trip-map sheet rows. Mid-stage shape-pill toggles can no longer flicker the card. Known pre-existing: `hasFlightStage` still keys off live tripShape (separate follow-up).
