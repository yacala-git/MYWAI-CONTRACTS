...
- `/build/hotel-recheck` + `/build/flight-recheck` need `LENS_HUB_STREAM_URL` on the API Lambda (box7) — it is NOT there by default (only box4-cognitive had it). Deploy passes `LensHubStreamUrl` in `deploy_box7`. Unset → hotel-recheck returns `{}` / flight-recheck returns `checked:false`. The LENS clients are vendored as `lambda/shared/lens_recheck.py` (hotel) + `lambda/shared/flight_recheck.py` (flight) (box7 packages `shared/`); neither is a drift-gated mirror. Deploy order: box1 (table) → box7 (IAM + routes + env + code)
