...
## The stay's because-clause — `(why, match_signals, why_inputs)` (2026-09-24, ticket 196)
The hotel card's reason was ONE string, `Because you {trait}: {property hook}.`, composed by
`sketch_engine._build_hotel_why` and split client-side on its first colon. It could carry exactly one
claim and could never say what the traveller ASKED for on this turn. **DELETED, no shim** —
`_build_hotel_why`, `_top_taste_trait`, `_property_hook`, `_spoken_amenity`, `_HOTEL_CODON_TRAIT`,
`_HOTEL_TASTE_MIN_AFFINITY` and `_MAX_HOTEL_WHY_CHARS` are all gone from `sketch_engine`.
- **`shared/match_reasons.py::build_hotel_reasons(intent_provenance, stated_phrases, stated_codons, codon_contribs, amenity_labels) -> (why, match_signals, why_inputs)`** — pure, no I/O, no LLM. Same triple as `_build_flight_why` / `_build_destination_why`.
- **Two registers, both SHERPA-side.** `_ASK_REGISTER` is keyed on SOURCE codons only (every `from` codon of the four DNA affinity graphs + the product codons a `graph="direct"` edge can carry); `_HOTEL_CODON_TRAIT` moved unchanged from `sketch_engine` and is the SAVED voice. BUDG#*/STAY.TIER#* are absent from BOTH.
- **At most TWO signals** (owner ruling): slot 1 the ask (strongest positive edge, else a requested amenity this hotel has), slot 2 the saved taste. The codon voiced in slot 1 is excluded from slot 2. `why == " · ".join(match_signals[:2])`.
- **Honesty rules, enforced structurally:** voice the SOURCE never the TARGET · a negative weight is never voiced · no register entry ⇒ NO signal (the only path from a codon to a string) · **"you asked" only with evidence the codon came from THIS turn: `stated_phrases` keys, or `stated_codons` = `session_intent_codons` ONLY.** `intent_codons` at that call site is `_genuine` off `taste_field.to_codon_hints()` — the SAVED DNA projection — so including it printed "Because you wanted somewhere romantic" for a traveller who said nothing (code review, 2026-09-24). Same one-line rule as the destination path's `_stated_codons`. `implied_intent_codons` is out too. · the verbatim span is NEVER rendered and `MatchReason.to_audit()` drops it, so `why_inputs` carries `stated: bool` and no raw traveller text.
- **An ENGINE-injected amenity is never voiced.** The family/business injection merges its display names into the same `extra_filters["amenities"]` the user's pills use, so `requested_codes` cannot tell them apart. `extra_filters["_engine_amenities"]` marks them; `asked_codes = requested_codes - engine_codes` is what may be voiced, while the card's match pills still use the full `requested_codes`. **Convention: an `_`-prefixed filter key is SHERPA-internal and is stripped before the filters reach DNA.**
- **One idea, one slot.** `_CLAUSE_IDEA` (codon → idea) and `_AMENITY_IDEA` (spoken label → idea) stop the two registers restating each other: "wanted a spa · look for a spa" is one reason spending both slots. The slot-2 exclusion is on the IDEA, not the codon.
- **Per-clause length cap `_MAX_CLAUSE_CHARS = 64`** — the card is `line-clamp-1`, so an overlong clause is DROPPED, never clipped mid-word. `register_clauses_too_long()` is the guard that keeps it firing only on a pathological amenity label (longest register clause today: 54 chars). The retired 120-char cap backed a TWO-line clamp and does not apply.
- **A span may only speak for a codon some other list still vouches for** (guru gate, 2026-09-24). `build_stated_phrases` reads BOTH enrichment buckets, but this path DISCARDS the strong bucket's codons (Sonnet owns intent here), so a strong-rule span outlived its codon and — because the ask gate is `keys(stated_phrases) ∪ session_intent_codons` — could license a provenance source that came from SAVED DNA alone (`to_codon_hints` emits `MOOD#ROMN` off `romance_mode > 0.2`). Reproduced: *"Romantic is not really our thing this trip - somewhere lively"* → `{MOOD#ROMN: 'Romantic'}` → "Because you wanted somewhere romantic", i.e. the card contradicting the sentence just typed. `_is_negated` cannot catch it (35-char lookbehind) and the romance veto lists `SOC#FAM`/`SOC#GRUP` only. **Fix:** after the fence AND after the veto, `stated_phrases` is filtered to `set(session_intent_codons) | set(implied_intent_codons)`. SOFT-rule spans are unaffected and still voice an ask ("I need a break from the kids" → "wanted to unwind") — known and accepted.

- **Copy withheld on purpose:** `PURP#MEDI` / `PURP#RELI` have NO ask entry — a medical or religious reason for travelling is not read back to the traveller. They still rank; only the voice is withheld. No clause presumes arrival (the card is read while planning).
- **`match_signals == []` ⇒ the card shows no reason and NO badge.** A valid outcome, never padded.
- **Threading:** `_dna_shortlist_candidates` reads `r["intent_provenance"]` (absent ⇒ `[]`) and takes a new `stated_phrases` kwarg from `_search_and_rank_hotels`; both candidate branches (`_hive_to_candidate` + the Aurora fallback) call the ONE composer; `_hotel_from_candidate` carries `match_signals` / `why_inputs` onto `HotelBlock` without recomposing.
- **INVARIANT:** ranking is untouched. `intent_provenance` is written DNA-side after every score mutation, no scorer reads it, and `codon_contribs` is unchanged (it is the inspector trace, a different job).
- **NOT built here:** the client render, and `intent_provenance` on the LENS wave-2 swap pool (a swapped card gets saved-taste reasons only).
