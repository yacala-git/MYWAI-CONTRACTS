...
- `partial: true` + `failed_stages[]`: a non-fatal stage failure still writes the rest of the record (re-attempted next month).
- **Fail-DEGRADED, not DLQ (Phase 1, 2026-07-15):** when the article can't be trusted — no resolvable/verified article, a **disambiguation** page, a **coord-mismatch** (article coords > `WIKIVOYAGE_COORD_THRESHOLD_KM`, default 200 km, from the row's GeoNames lat/lng), or a **None blurb** — the worker WRITES an identity-safe landmark-only row (deterministic minimal blurb `"Name, Region, Country"`, geo-grounded landmark codons or the minimal-codon floor, `wikivoyage=false`, low `confidence`≈0.3, `partial=true`) through the normal write→version-bump→sync path. This **overwrites existing poison** and rebuilds the vector — the self-heal the old fail-closed path lacked. Only a **TRANSIENT** failure redrives and is NOT written: a Wikivoyage fetch **exception** (network / MediaWiki 200-with-`{"error"}` envelope, which `_api_get` RAISES) or an unkeyable message.
- **Phase 1 identity guards (no new corrupt rows):** (1a) the authoritative enwikivoyage article is resolved from `geoname_id` via Wikidata `P1566` → `wbgetentities` sitelink and used over the message `wikivoyage_title` (falls back to the message title on no-item / transient error; genuine negatives cached, transient errors NOT cached); (1b) disambiguation (`pageprops.disambiguation`, authoritative; lead regex fallback) + coord cross-check reject in `wikivoyage.fetch`; (1c) the blurb stage enforces a strict JSON output-contract `{"ok": bool, "blurb": str}` (`ok` must be boolean `true`) + a refusal/meta-language lexicon anchored to the first ~40 chars → `None` → minimal blurb (never the raw `source_text` head); (1d) a zero-landmark geoname still gets a writable non-null-vector row via the minimal-codon floor (`DEST#URBN`).
